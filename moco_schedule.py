...
"""1) train_transform의 순서 바꿔보기 -> -1%
2) stochastic process로 바꾸기
epoch마다 다른 transformation.
dataloader에서는 원본 이미지를 가져오고, train 함수 안에서 rand를 이용해 transformation한다.
3) test_transform도 train_transform처럼 변경
4) 다른 augmentations 추가( Cut out 추가하면 어떻게 되려나?)
5) Multi-crop으로도 돌려보기
...
train_transform_2 = transforms.Compose([
    ...
    transforms.RandomResizedCrop(32,scale=(0.08,0.5)),
    transforms.RandomHorizontalFlip(p=0.5),
    transforms.RandomApply([transforms.ColorJitter(1, 1, 1, 0.35)], p=0.8),
...
train_loader_2 = DataLoader(train_data_2, batch_size=args.batch_size, shuffle=True, num_workers=16, pin_memory=True, drop_last=True)
...
# train for one epoch
def train(net, data_loader_1,data_loader_2, train_optimizer, epoch, args):
    net.train()
    ...
    if(epoch>100):
      ...
      total_loss, total_num, train_bar = 0.0, 0, tqdm(data_loader_1)

    for im_1, im_2 in train_bar:
        # experiment 
        # 512 x 3 x 32 x 32(torch.size)   
        ...
        total_num += data_loader_1.batch_size
        total_loss += loss.item() * data_loader_1.batch_size
...
for epoch in range(epoch_start, args.epochs + 1):
    train_loss = train(model, train_loader, train_loader_2, optimizer, epoch, args)
